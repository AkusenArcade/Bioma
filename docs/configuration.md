...
| `session.lock` / `session.suspend` / `session.restart` / `session.shutdown` / `session.logout` | Each is a list — the program and its arguments. None of the five is universal: a lock screen is a choice, systemd is not the only init, and leaving the session is a different sentence on every compositor. The defaults are `systemctl suspend|reboot|poweroff` and `niri msg action quit -s`. Lock has none: Bioma has no lock screen of its own yet, so LOCK is shown unavailable until this names a locker, for example `["hyprlock"]`. |
