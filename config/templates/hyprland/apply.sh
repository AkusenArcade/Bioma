...
lua_output_file="$config_dir/bioma.lua"
conf_output_file="$config_dir/bioma.conf"
...
if [[ "$config_dir" == "$HOME"/* ]]; then
  conf_source_path="~/${config_dir#"$HOME"/}/bioma.conf"
else
...
detect_mode() {
  ...
  # Fallback for cases where bioma applies templates where hyprland
...
apply_lua() {
  local include_line='-- For Bioma Color templates
require("bioma").apply_theme()'
  ...
  # Append only if there is no Bioma include at all
  if ! grep -qF 'require("bioma")' "$lua_config_file"; then
    printf '\n%s\n' "$include_line" >>"$lua_config_file"
  fi
}
...
source = $conf_source_path"
  ...
  # Avoid appending duplicate Bioma includes
  if ! grep -qE 'source\s*=.*bioma\.conf' "$conf_config_file"; then
    printf '\n%s\n' "$include_line" >>"$conf_config_file"
  fi
}
