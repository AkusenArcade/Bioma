...
GTK_IMPORT='@import url("bioma.css");'
...
ensure_gtk_css_import() {
    ...
    if [ ! -f "$colors_file" ]; then
        echo "Error: $label bioma.css not found at $colors_file" >&2
        return 1
    fi
    ...
    if [ -e "$gtk_css" ] || [ -L "$gtk_css" ]; then
        local content
        content=$(cat "$gtk_css")
        if [[ "$content" == *"bioma.css"* ]] && [[ "$content" == *"@import"* ]]; then
            return 0
        fi
        ...
        echo "Appended $label bioma.css import to gtk.css"
    else
        printf '%s\n' "$GTK_IMPORT" > "$gtk_css"
        echo "Created $label gtk.css with bioma.css import"
    fi
}
