...
awk '
    ...
    # dropping any existing bioma.toml entry and appending it last so it
    ...
    function build_optional(buf,   open, endp, head, inner, tail, test, multiline, indent) {
        ...
        gsub(/"bioma\.toml"[[:space:]]*,[[:space:]]*/, "", inner)
        gsub(/,[[:space:]]*"bioma\.toml"/, "", inner)
        gsub(/"bioma\.toml"/, "", inner)
        ...
        if (test == "")
            return head "\"bioma.toml\"" tail
        sub(/[[:space:]]+$/, "", inner)
        return head inner ", \"bioma.toml\"" tail
    }

    # Remove the generated include from the mandatory section written by older
    # Bioma versions while preserving every user-owned include.
    function build_required(buf,   open, endp, head, inner, tail) {
        if (index(buf, "\"bioma.toml\"") == 0)
            return buf
