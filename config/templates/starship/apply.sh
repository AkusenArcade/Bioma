...
palette_file="${XDG_CACHE_HOME:-$HOME/.cache}/bioma/starship-palette.toml"
marker_begin="# >>> BIOMA STARSHIP PALETTE >>>"
marker_end="# <<< BIOMA STARSHIP PALETTE <<<"
...
discover_starship_config() {
    ...
    # Bioma applies templates from its daemon, which does not inherit shell-only
...
if [ ! -f "$config_file" ]; then
    ...
    # Strip the previous bioma palette block and any palette= line, keep the rest.
