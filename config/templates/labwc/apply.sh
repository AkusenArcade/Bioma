...
theme_dir="${XDG_DATA_HOME:-$HOME/.local/share}/themes/bioma/openbox-3"
theme_file="$theme_dir/themerc"
source_theme="$config_dir/bioma.conf"
...
if grep -q '<theme>' "$rc_file"; then
    ...
    if sed -n '/<theme>/,/<\/theme>/ {
        ...
        sed '/<theme>/,/<\/theme>/ {
            /<font[[:space:]>].*<\/font>/b
            /<font[[:space:]>]/,/<\/font>/b
            s|<name>.*</name>|<name>bioma</name>|
        }' "$rc_file" >"$tmp_file"
    else
        sed '/<theme>/a\    <name>bioma</name>' "$rc_file" >"$tmp_file"
    fi
else
