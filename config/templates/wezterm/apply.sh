...
scheme_line='config.color_scheme = "Bioma"'
...
if ! grep -q "^\s*config\.color_scheme\s*=\s*['\"]Bioma['\"]\s*" "$config_file"; then
    tmp_file="$(mktemp "${config_file}.tmp.XXXXXX")"
    trap 'rm -f "$tmp_file"' EXIT

    if grep -q '^\s*config\.color_scheme\s*=' "$config_file"; then
        sed "s|^\(\s*config\.color_scheme\s*=\s*\).*$|\1\"Bioma\"|" "$config_file" >"$tmp_file"
        trap - EXIT
