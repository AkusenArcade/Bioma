...
output_file="$config_dir/bioma-theme.kdl"
include_line='include "bioma-theme.kdl"'

has_bioma_include() {
    grep -Eq '^[[:space:]]*include([[:space:]].*)?"([^"]*/)?bioma-theme\.kdl"([[:space:]]|$)' "$config_file"
}
